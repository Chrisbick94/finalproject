...
app.post("/create-account", upload.single("photo"), ourCleanup, async (req, res) => {
  if (req.file) {
  ...
  const info = await db.collection("accounts").insertOne(req.cleanData)
  const newAnimal = await db.collection("accounts").findOne({ _id: new ObjectId(info.insertedId) })
  res.send(newAnimal)
})
...
app.delete("/account/:id", async (req, res) => {
  ...
  db.collection("accounts").deleteOne({ _id: new ObjectId(req.params.id) })
  res.send("Good job")
})

app.post("/update-accounts", upload.single("photo"), ourCleanup, async (req, res) => {
  if (req.file) {
    // if they are uploading a new photo
    const photofilename = `${Date.now()}.jpg`
    await sharp(req.file.buffer).resize(844, 456).jpeg({ quality: 60 }).toFile(path.join("public", "uploaded-photos", photofilename))
    req.cleanData.photo = photofilename
    const info = await db.collection("accounts").findOneAndUpdate({ _id: new ObjectId(req.body._id) }, { $set: req.cleanData })
    if (info.value.photo) {
    ...
    db.collection("accounts").findOneAndUpdate({ _id: new ObjectId(req.body._id) }, { $set: req.cleanData })
    res.send(false)
  }
})
...
async function start() {
  const client = new MongoClient("mongodb://root:root@localhost:27017/FinalProject?&authSource=admin")
